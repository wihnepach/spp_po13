...
class CollaborationCollector:
    ...
    def to_dict(self):
        return {
            ...
            "interaction_details": {user: dict(interactions) for user, interactions in self.edge_details.items()},
        }
...
class GraphVisualizer:
    ...
    def visualize(self, output_filename="github_graph.png", figsize=(15, 10)):
        ...
        nx.draw_networkx_nodes(
            self.graph, pos, nodelist=[self.username], node_color="#e74c3c", node_size=3000, node_shape="o"
        )
        ...
        for edge in self.graph.edges(data=True):
            if len(edge) == 3:
                ...
                nx.draw_networkx_edges(
                    self.graph, pos, edgelist=[(u, v)], width=weight * 2, edge_color=color, alpha=0.6, style="solid"
                )

        nx.draw_networkx_labels(
            self.graph, pos, {node: node for node in self.graph.nodes()}, font_size=10, font_weight="bold"
        )
        ...
        for interaction_type, color in self.EDGE_COLORS.items():
            ...
            legend_elements.append(plt.Line2D([0], [0], color=color, lw=4, label=display_name, alpha=0.6))
...
def main():
    ...
    collector = InteractionCollector(
        username=username, api_client=api_client, collaboration_collector=collaboration_collector
    )
    ...
    json_data = {"username": username, "repositories": repos_list, "collaborators": collaboration_collector.to_dict()}
